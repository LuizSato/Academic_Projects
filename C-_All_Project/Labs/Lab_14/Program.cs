...
namespace Lab_14
{
    class Program
    {
        ...
        {
            AddElement();
            //WriteOneToXml(elements[0], "OneToXml.xml");
            //ReadOneToXml("OneToXml.xml");
            //WriteAllToXml(elements, "AllToXml.xml");
            //ReadAllToXml("AllToXml.xml");
            //ReadAllToXml("AllToXml.xml");
            //WriteOneToJson(elements[0], "WriteOneToJson.json");
            //ReadOneToJson("WriteOneToJson.json");
            WriteAllToJson(elements, "WriteAllToJson.txt");
            ReadAllToJson("WriteAllToJson.json");

        }
        static void WriteOneToXml(Atom element, string filename)
        {
        ...
        static void ReadOneToXml(string filename)
        {
            using(TextReader reader = new StreamReader(filename))
            {
                XmlSerializer serializer = new XmlSerializer(typeof(Atom));
                Atom elem = (Atom)serializer.Deserialize(reader);
                Console.WriteLine($"{elem}");
            }
        }
        static void WriteAllToXml(List<Atom> element, string filename)
        {
            using(TextWriter writer = new StreamWriter(filename))
            {
        ...
        static void ReadAllToXml(string filename)
        {
            using(TextReader reader = new StreamReader(filename))
            {
                XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
                List<Atom> elem = (List<Atom>)serializer.Deserialize(reader);
                foreach (Atom x in elem)
                {
                    Console.WriteLine($"{x}");
                }
        ...
        static void WriteOneToJson(Atom element, string filename)
        {
            using(TextWriter writer = new StreamWriter(filename))
            {
        ...
        static void ReadOneToJson(string filename)
        {
            using (TextReader reader = new StreamReader(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Atom elem = serializer.Deserialize<Atom>(reader.ReadLine());
                Console.WriteLine($"{elem}");
            }
        }
        static void WriteAllToJson(List<Atom> element, string filename)
        {
            using(TextWriter writer = new StreamWriter(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
        ...
        static void ReadAllToJson(string filename)
        {
            using(TextReader reader = new StreamReader(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                List<Atom> elem = serializer.Deserialize<List<Atom>>(reader.ReadLine());
                foreach (Atom x in elem)
                {
                    Console.WriteLine($"{x}");
                }
        ...
        static void AddElement()
        {
            try
            {
                ...
                elements.Add(Atom.Parse("hello 102 102 102 102 102"));
            }
            catch (Exception e)
            {
